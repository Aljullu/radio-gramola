...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
	<?php
		...
		if ($page_exploded[2] == "serie") {
			$serie = $page_exploded[3];
		}
		else if ($page_exploded[2] == "cancons-sobre") {
		    $tema = $page_exploded[3];
		}
		if (isset($serie)) {
		...
		$arrayserie = mysqli_fetch_array($infoserie)
	?>
...
<meta name="description" content="Llista de cançons de <?php echo $arrayserie['nom']; ?> amb les lletres, videoclips, informació dels grups, artistes i àlbums." />
	<?php
		...
		$arraytema = mysqli_fetch_array($infotema)
	?>
...
<script src="<?php echo $baseUrl; ?>/js/jquery.quicksearch.js"></script>
<script>
$(document).ready(function() {
	$('input#cercador').quicksearch('table#llista tbody tr');
	if (window.external.IsSearchProviderInstalled('<?php echo $baseUrl; ?>/opensearch.xml') == 0) {
		$('#opensearch').show();
	}
});
</script>
...
<div id="content">
	...
	<p>Podeu veure, també, la <a href="<?php echo $baseUrl; ?>/llista-cancons.php?mes=novembre-2011" title="Llista de cançons en català" rel="nofollow">llista de cançons amb els vots del novembre</a>, el <a href="<?php echo $baseUrl; ?>/llista-cancons.php?mes=desembre-2011" rel="nofollow">desembre</a>, el <a href="<?php echo $baseUrl; ?>/llista-cancons.php?mes=gener-2012" rel="nofollow">gener</a>, el <a href="<?php echo $baseUrl; ?>/llista-cancons.php?mes=febrer-2012" rel="nofollow">febrer</a>, el <a href="<?php echo $baseUrl; ?>/llista-cancons.php?mes=marc-2012" rel="nofollow">març</a> i l'<a href="<?php echo $baseUrl; ?>/llista-cancons.php?mes=abril-2012" rel="nofollow">abril</a>.</p>
	<form name="input" action="#" method="get">
		...
		<p id="opensearch" style="display:none;" ><small><a href="javascript:void(0)" onClick="window.external.AddSearchProvider('<?php echo $baseUrl; ?>/opensearch.xml');">Instal·leu-vos aquest cercador al navegador.</a></small></p>
	</form>
	<?php
		} elseif (isset($tema)) {
        // Temes
	    ...
	    $infopopulartemes = mysqli_query($con, $query); 
		$texttoprint = "";
		$first = true;
		while ($populartema = mysqli_fetch_array($infopopulartemes)) {
			if (!$first) $texttoprint .= ', ';
			else $first = false;
			$texttoprint .= '<a href="'. $baseUrl .'/cancons-sobre/'. $populartema['uri'] .'" title="Cançons de '. $populartema['nom'] .'">'. $populartema['nom'] .'</a>';
		}
	?>
...
<table id="llista" class="sortable">
	...
	<?php
		...
		while ($canco = mysqli_fetch_array($infocanco)) {
				...
				echo "<td><a href='". $baseUrl ."/d/".$canco['artistauri']."/".$canco['uri']."'>";
				echo "<meta itemprop='url' content='". $baseUrl ."/d/".$canco['artistauri']."/".$canco['uri']."'>";
				...
				echo "<td><a href='". $baseUrl ."/d/".$canco['artistauri']."'>".$canco['artista']."</a></td>";
